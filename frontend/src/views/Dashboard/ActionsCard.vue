...
const submitApplication = async () => {
  if (!formData.value.applicantName) {
    toast.error('Application Name is missing');
  }
  if (!formData.value.applicantEmail) {
    toast.error('Application Email is missing');
  }
  if (!formData.value.applicantMobilePhoneNumber) {
    toast.error('Application Mobile Phone Number is missing');
  }
  if (!formData.value.applicantAddress) {
    toast.error('Application Address is missing');
  }
  if (!formData.value.annualIncomeBeforeTax) {
    toast.error('Annual Income Before Tax is missing');
  }
  if (!formData.value.incomingAddress) {
    toast.error('Incoming Address is missing');
  }
  if (!formData.value.incomingDeposit) {
    toast.error('Incoming Deposit is missing');
  }
  if (!formData.value.incomingPrice) {
    toast.error('Incoming Price is missing');
  }
  if (!formData.value.incomingStampDuty) {
    toast.error('Incoming Stampduty is missing');
  }
  if (!formData.value.loanAmount) {
    toast.error('Loan Amount is missing');
  }
  if (!formData.value.loanDuration) {
    toast.error('Loan Duration is missing');
  }
  if (!formData.value.monthlyExpenses) {
    toast.error('Monthly Expenses is missing');
  }
  if (!formData.value.outgoingAddress) {
    toast.error('Outgoing Address is missing');
  }
  if (!formData.value.outgoingMortgage) {
    toast.error('Outgoing Mortage is missing');
  }
  if (!formData.value.outgoingValuation) {
    toast.error('Outgoing Valuation is missing');
  }
  if (!formData.value.savingsContribution) {
    toast.error('Saving Contribution is missing');
  }
  if (!formData.value.applicantName || !formData.value.applicantEmail || !formData.value.applicantMobilePhoneNumber || !formData.value.applicantAddress || !formData.value.annualIncomeBeforeTax || !formData.value.incomingAddress || !formData.value.incomingDeposit || !formData.value.incomingPrice || !formData.value.incomingStampDuty || !formData.value.loanAmount || !formData.value.loanDuration || !formData.value.monthlyExpenses || !formData.value.outgoingAddress || !formData.value.outgoingMortgage || !formData.value.outgoingValuation || !formData.value.savingsContribution) {
    return;
  }
